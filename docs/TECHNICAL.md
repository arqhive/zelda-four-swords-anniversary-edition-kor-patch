...
- `tools/dev/polish_graphics.py`: v1.0.1 롬을 기준으로 수련장 잔여 픽셀 → 파일 관리 안내 → 공통 버튼·대기·색 선택 → 멀티플레이 메뉴를 단계별 수정. 각 단계에서 ROM/BPS/비교 이미지/검증 기록 생성.
- `Back`은 메뉴 NCGR 두 사본 외에 `zeldat_us_en.bin` 항목 16에도 존재한다. 이 사본은 글자 윗부분과 아랫부분의 타일이 분리되어 있어 별도로 대응시킨다.
