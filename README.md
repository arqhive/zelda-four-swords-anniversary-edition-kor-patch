...
- v1.1 그래픽: 사용자 에뮬레이터 실행 확인, 로고 수정본은 사용자 실기 표시 확인 완료

**v1.1 배포 파일 갱신:** 큰 제목 로고가 영문으로 남던 문제를 수정하고 사용자 실기 확인을 마쳤습니다. 버전은 v1.1로 유지합니다. 기존 v1.1을 받았다면 파일을 다시 내려받아 북미판 원본에 적용하세요. [원인과 수정 내역](docs/TITLE_LOGO_FIX.md).
...
위 결과 해시는 v1.1 배포 BPS를 적용한 롬 기준입니다. 파일 재배치·빈 공간의 이전 데이터 및 폰트 환경에 따라서도 전체 롬 해시가 달라질 수 있습니다. 갱신한 v1.1 소스 빌드와 배포본의 전체 25개 NitroFS 파일 내용이 같음을 검증했습니다. 로고 원본 이미지를 별도로 준비하거나 ImageGen을 실행할 필요는 없습니다.
